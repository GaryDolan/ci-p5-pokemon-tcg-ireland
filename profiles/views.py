    ...
    if request.method == 'POST':
        if 'edit_acc_info' in request.POST:
            ...
            if user_account_form.is_valid():
                ...
                messages.error(
                    request,
                    'An error occurred updating your info, please try again'
                )
            ...
            if user_profile_form.is_valid():
                ...
                messages.error(
                    request,
                    'An error occurred updating your shipping info, '
                    'please try again'
                )
        ...
        # Render the template only initialising forms that have not failed
        # validation. If we failed a form submission return the same
        # instance of the form to display error in validation.
        user_account_form = UserAccountForm(
            instance=request.user
        ) if not acc_form_failed else user_account_form
        user_profile_form = UserProfileForm(
            instance=profile
        ) if not profile_form_failed else user_profile_form
        user_password_form = PasswordChangeForm(
            request.user
        ) if not password_form_failed else user_password_form
...
def order_history(request, order_number):
    """
    View to handel displaying order history
    """
